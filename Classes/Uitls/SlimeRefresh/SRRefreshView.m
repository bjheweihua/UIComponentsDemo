...
@property (nonatomic, assign)   BOOL    endAnimal;//默认为yes  当 退出当前页面的时候  设为no
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _slime.startPoint = CGPointMake(frame.size.width / 2, CGRectGetHeight(frame) / 2);
        ...
        _dragingHeight = CGRectGetHeight(frame);
        _endAnimal = YES;
...
{
    CGRect frame = CGRectMake(0, 0, kMainScreenW, height);
    return  [self initWithFrame:frame];
}
